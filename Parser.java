...
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
        ...
        for ( int i = 0; i < nodeList.getLength(); i++ ) {
            ...
            } else if ( nodeName.equalsIgnoreCase("cone")) {
            	Cone cone = Parser.createCone(n);
            	scene.surfaceList.add(cone);
            }
	...
	public static Light createLight(Node dataNode) {
        ...
        Node areaAttr = dataNode.getAttributes().getNamedItem("isAreaLight");
        if ( areaAttr != null ) {
        	light.isAreaLight = Boolean.parseBoolean(areaAttr.getNodeValue());
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
	    	...
	    	Node perlinAttr = dataNode.getAttributes().getNamedItem("isPerlin");
	    	if ( perlinAttr != null ) {
	    		material.isPerlin = Boolean.parseBoolean( perlinAttr.getNodeValue() );
	    	}
	    	Node reflectiveAttr = dataNode.getAttributes().getNamedItem("isReflective");
	    	if ( reflectiveAttr != null ) {
	    		material.isReflective = Boolean.parseBoolean( reflectiveAttr.getNodeValue() );
	    	}
	    	Node refractiveAttr = dataNode.getAttributes().getNamedItem("isRefractive");
	    	if ( refractiveAttr != null ) {
	    		material.isRefractive = Boolean.parseBoolean( refractiveAttr.getNodeValue() );
	    	}
		}
		return material;
	}
	 ...
	 */
	public static Cone createCone(Node dataNode) {
		Cone cone = new Cone();
		Node centerAttr = dataNode.getAttributes().getNamedItem("center");
		if ( centerAttr != null ) {
            Scanner s = new Scanner( centerAttr.getNodeValue() );
            ...
            double y = s.nextDouble();
            double z = s.nextDouble();
            cone.center = new Point3d(x, y, z);
            s.close();
		}
		Node radiusAttr = dataNode.getAttributes().getNamedItem("radius");
		if ( radiusAttr != null ) {
			cone.radius = Double.parseDouble( radiusAttr.getNodeValue() );
		}
		Node heightAttr = dataNode.getAttributes().getNamedItem("height");
		if ( heightAttr != null ) {
			cone.height = Double.parseDouble( heightAttr.getNodeValue() );
		}
		cone.material = parseMaterial(dataNode, "material");	
    	return cone;
	}
